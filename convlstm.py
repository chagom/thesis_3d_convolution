...
frame_files = sorted([os.path.join(data_file_path, file)
    for file in os.listdir(data_file_path)
    if (os.path.splitext(os.path.join(data_file_path, file))[0]).split('_')[-1] == 'data'])

label_files = sorted([os.path.join(data_file_path, file)
    for file in os.listdir(data_file_path)
    if (os.path.splitext(os.path.join(data_file_path, file))[0]).split('_')[-1] == 'label'])

X = []
y = []

for frame, label in zip(frame_files, label_files):
    X.append(np.load(frame, allow_pickle = True))
    y.append(np.load(label, allow_pickle = True))
'''
...
def ConvLSTMModel_Test():
    ...
    model.add(layers.ConvLSTM2D(filters=64, kernel_size=(3,3), return_sequences=True, data_format="channels_last", input_shape=(90, 64, 64, 1)))
...
def D3Net():
    ...
    model.add(tf.keras.layers.Conv3D(128, (3,3,3), activation='relu', input_shape=(64,64,1)))
...
filepath = "save-model-{epoch:02d}-{val_accuracy:.2f}.hdf5"
...
for train, test in kfold.split(inputs, targets):
    ...
    #model.compile(loss=loss_function, optimizer=optimizer, metrics=['accuracy'], options = run_opts)
...
'''
    ...
    history = model.fit(train_x, target_train, batch_size = batch, epochs=epoch, verbose=verbosity)

    score = model.evaluate(test_y, target_test, verbose=0)
